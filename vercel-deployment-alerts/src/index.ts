...
export async function sendCampsiteAlert(
	title: string,
	content: string
): Promise<CampsiteResponse> {
	if (!CAMPSITE_API_KEY) {
		throw new Error(
			"CAMPSITE_API_KEY is not defined in the environment variables"
		);
	}
...
## Project Details
- **Project Name**: ${
		payload.name ? `[${payload.name}](${payload.url || ""})` : "N/A"
	}
- **Deployment URL**: ${
		payload.url ? `[${payload.url}](https://${payload.url})` : "N/A"
	}
- **Inspect Deployment**: ${
		payload.deployment?.inspectorUrl
			? `[View on Vercel](${payload.deployment.inspectorUrl})`
			: "N/A"
	}
...
${
	regions.length > 0
		? regions.map((region: string) => `- ${region}`).join("\n")
		: "N/A"
}
